...
这个仓库开发一款面向 Codex 长编程任务的本地项目记忆插件。它通过 SQLite FTS5、生命周期 Hooks 和 MCP 工具保存版本化项目计划、当前工作项、精确下一步、带来源的工程记忆、失败经验、验证证据与压缩前检查点。
...
项目记忆会在 `PreCompact` 确定性生成 Mainline Capsule 并保存幂等检查点，在压缩后的 `SessionStart(source=compact)` 根据 SQLite 与当前 Git 状态立即重建并注入项目主线。它不会递归总结上一次压缩摘要，不会把完整历史常驻到模型上下文，也不需要 API Key。

注入内容包含项目目标、计划版本、里程碑、当前工作项、精确下一步、阻塞、关键约束、待用户决策、验证新鲜度和 Git 状态。正常注入最多 6500 字符，压缩恢复最多 6000 字符，适合将 128k/256k 上下文作为工作区而不是长期存储。

## 清除当前项目记忆

在 Codex 输入框键入 `/`，选择 **Reset Project Memory**（技能名 `reset-project`）。该命令只会在用户明确执行时触发，并通过两次相同的项目根目录校验，删除该项目的计划、任务、记忆、事件、验证记录、检查点、胶囊和导出文件。
