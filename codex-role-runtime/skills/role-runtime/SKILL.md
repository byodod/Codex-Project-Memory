---
name: role-runtime
description: Keep persistent project roles independent from replaceable Codex thread generations. Use when the user asks to define roles, assign semantic ownership, route work between roles, use typed role messages, inspect a role mailbox or task graph, enforce role permissions, start or rotate a role session, recover a stale role thread, or orchestrate long modular development with Coordinator, Architect, Module Owner, Worker, and Verifier responsibilities. Do not use for ordinary short tasks that need no persistent role topology.
---
...
2. If no roles exist, define a short L0 constitution with `project_configure`.
3. Define only the needed long-lived roles with `role_define`. Start with Coordinator, Architect, and Verifier; add Module Owners only for real bounded contexts.
4. Long-lived governance and owner roles should normally be `read_only`. Use short-lived `workspace_write` workers for implementation.
5. Record critical invariants, accepted decisions, ownership, open questions, failures, and artifacts with `role_state_put`. Do not store transcript summaries as role truth.

The bundled CLI can create the standard topology in one command:

`node <plugin-root>/dist/cli.mjs init --cwd <project>`

## Bind or create a generation

- For an existing unbound Codex thread, the user can submit exactly `role://bind <role-key>`. The `UserPromptSubmit` hook binds generation 1 if the role is unbound.
...
1. Keep the Coordinator compact: use `task_upsert` and `task_graph`; store pointers and blockers, not all module knowledge.
...
- `SessionStart` and `UserPromptSubmit` inject a compact role anchor.
