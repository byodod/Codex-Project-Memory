...
var SECRET_KEY = /(?:secret|token|password|passwd|api[_-]?key|authorization|cookie|private[_-]?key)/i;
var SECRET_VALUE = [
...
function redact(value, depth = 0) {
  ...
  if (typeof value === "string") return redactString(compactText(value, 12e3));
...
function fallbackSessionStart(input) {
  ...
  const budget = input.source === "compact" ? 6e3 : 6500;
...
function extractError(value) {
  const text = compactText(value, 8e3);
  const lines = text.split("\n").filter((line) => /\b(error|exception|failed|failure|fatal|CS\d{4}|TS\d{4})\b/i.test(line));
  return lines.length ? compactText(lines.slice(0, 3).join(" | "), 1e3) : null;
}
    ...
    switch (input.hook_event_name) {
      case "SessionStart": {
        ...
        const curated = store.search(project, "", {
          taskId: task?.id,
          kinds: ["decision", "constraint", "project_fact", "tool_quirk"],
          limit: 4
        });
        const mainline = renderMainlineCapsule(capsule, budget);
        const remaining = Math.max(0, budget - mainline.length - 2);
        const context = [mainline, remaining >= 400 ? renderMemories(curated, "Active curated memory", remaining) : ""].filter(Boolean).join("\n\n").slice(0, budget);
      ...
      case "UserPromptSubmit": {
        ...
        const memories = store.search(project, compactText(safePrompt, 4e3), { taskId: task?.id, limit: 6 });
      ...
      case "PostToolUse": {
        ...
        store.recordEvent(project, {
          ...
          payload: { tool_name: input.tool_name, input: safeInput, response: safeResponse },
          exitCode,
          filePath: extractFile(input.tool_input),
        ...
        if (error && exitCode !== 0) {
          store.storeMemory(project, {
            task_id: task?.id,
            kind: "episodic",
            summary: `Observed failure in ${input.tool_name ?? "tool"}`,
            content: error,
            authority: "tool_observation",
            confidence: 0.95,
            importance: 0.35,
            source_note: input.tool_use_id ? `tool_use_id=${input.tool_use_id}` : "PostToolUse",
            file_path: extractFile(input.tool_input) ?? void 0,
...
`);
      ...
      case "PostCompact": {
        store.recordEvent(project, {
          ...
          payload: { trigger: input.trigger ?? "unknown" },
          authority: "tool_observation"
        });
        return null;
      }
      case "Stop": {
        if (input.last_assistant_message) {
          store.recordEvent(project, {
            ...
            sessionId: input.session_id,
            turnId: input.turn_id,
            eventType: "assistant_stop",
            payload: redact(input.last_assistant_message),
