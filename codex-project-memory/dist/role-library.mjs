...
function completeAttachment(store, project, roleKey, threadId, reason, rotation, candidate) {
...
function attachRoleThread(store, project, roleKey, threadId, reason = "Attached by Codex desktop task orchestration.") {
  const role = store.getRole(project, roleKey);
  if (!role) throw new Error(`Unknown role: ${roleKey}`);
  ...
  const openRotation = store.openRotation(project, roleKey);
  ...
  try {
    ...
    if (candidate) {
      try {
        store.rejectCandidate(candidate.id, `Task attachment failed: ${error instanceof Error ? error.message : String(error)}`);
      } catch {
      }
    }
    try {
...
function recipientRoute(store, project, roleKey) {
  const generation = store.activeGeneration(project, roleKey);
  return generation ? { role: roleKey, status: "active", thread_id: generation.thread_id, generation: generation.generation_number } : { role: roleKey, status: "needs_task", thread_id: null, generation: null };
}
function routeRoleMessage(store, project, input) {
...
function prepareLiaisonRequest(store, project, input) {
...
function recordLiaisonResult(store, project, input) {
