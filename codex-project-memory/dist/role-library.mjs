...
async function replaceMissingActiveGeneration(store, project, roleKey, missing, options) {
  ...
  const result = await rotateRoleGeneration(store, project, roleKey, `Active App Server task ${current.thread_id} no longer exists.`, options);
...
function handoffRoleGenerationToThread(store, project, roleKey, threadId, reason) {
  const role = store.getRole(project, roleKey);
  if (!role) throw new Error(`Unknown role: ${roleKey}`);
  ...
  try {
    ...
    if (candidate) {
      try {
        store.rejectCandidate(candidate.id, `Existing-thread handoff failed: ${error instanceof Error ? error.message : String(error)}`);
      } catch {
      }
    }
    try {
...
var AUTO_WAKE_TYPES = /* @__PURE__ */ new Set(["ASSIGN", "VERIFY_REQUEST", "HANDOFF"]);
async function dispatchRoleMessage(store, project, input, options = {}) {
...
async function dispatchLiaisonRequest(store, project, input, options = {}) {
  ...
  const runCoordinator = async (generation) => {
    const client = options.clientFactory ? await options.clientFactory() : await AppServerClient.connect();
    try {
      return await client.runTurn(generation.thread_id, prompt, options.timeoutMs);
    } finally {
      client.close();
    }
  };
